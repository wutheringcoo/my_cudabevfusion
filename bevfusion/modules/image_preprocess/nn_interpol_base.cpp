...
NearestNeighborInterpol::NearestNeighborInterpol(const BevFusionParams& params) :
        ...
        dst_c_(params_.normparams.resizeparams.dst_image_channel),
...
unsigned char* NearestNeighborInterpol::forward(const unsigned char* src_img, unsigned char* dst_img) {
    //
    ...
    for (int i = 0; i < dst_h_; ++i) {
        for (int j = 0; j < dst_w_; ++j) {
            for (int k = 0; k < dst_c_; ++k) {
                int src_y = i / yscale_;
                int src_x = j / xscale_;
                dst_img[i * dst_w_ * dst_c_ + j * dst_c_ + k] = src_img[src_y * src_w_ * dst_c_ + src_x * dst_c_ + k];
